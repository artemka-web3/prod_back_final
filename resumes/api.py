...
def suggestResumePdf(request, pdf: UploadedFile = File(...)):
    ...
    with GigaChat(credentials=GIGA_TOKEN, verify_ssl_certs=False) as giga:
        data = giga.chat('Я тебе предоствалю резюме, тебе нужно вычленить из него хард-скилы, софт-скилы, bio. Напиши кратко, только самое основное (не больше 2000 символов). Количество хард-скилов должно быть не больше 4, колиество софт-скилов должно быть не больше 4. Резюме: '+text+' Результат верни в формате JSON-списка без каких-либо пояснений, например, {"bio": "bio", "hards": ["skill1"], "softs": ["skill1"]}. Не повторяй фразы из примера и не дублируй фразы.').choices[0].message.content
        data = json.loads(data)
        return 200, data