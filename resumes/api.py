...
def suggestResumeHH(request, hh_link: SuggestResumeSchema):
    ...
    data = giga_chat.chat('Я тебе предоствалю резюме из hh.ru, тебе нужно выленить из него хард-скилы, софт-скилы, bio. Резюме: ' + bs.text + ' Результат верни в формате JSON-списка без каких-либо пояснений, например, {"bio": "bio", "hards": ["skill1"], "softs": ["skill1"]}. Если ты не смог найти какое-то поле, то поставь его значение null. Не повторяй фразы из примера и не дублируй фразы.').choices[0].message.content
